...
#cd && curl https://raw.githubusercontent.com/catonrug/zabbix-scripts/master/CentOS/custom-server-4.0-percona-mysql.sh > install.sh && chmod +x install.sh && time ./install.sh
...
mkdir -p ~/.ssh
# install vagrant ssh key for passwordless access
