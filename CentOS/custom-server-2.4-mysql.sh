...
#cd && curl https://raw.githubusercontent.com/catonrug/zabbix-scripts/master/CentOS/custom-server-3.0-mysql.sh > install.sh && chmod +x install.sh && time ./install.sh 2.4.8
...
yum install zabbix-server-mysql-$v -y
if [ $? -ne 0 ]; then
echo zabbix-server-mysql package not found
else

#create zabbix database structure
ls -l /usr/share/doc/zabbix-server-mysql*/create
cat /usr/share/doc/zabbix-server-mysql*/create/schema.sql | mysql -uzabbix -pTaL2gPU5U9FcCU2u zabbix
cat /usr/share/doc/zabbix-server-mysql*/create/images.sql | mysql -uzabbix -pTaL2gPU5U9FcCU2u zabbix
cat /usr/share/doc/zabbix-server-mysql*/create/data.sql | mysql -uzabbix -pTaL2gPU5U9FcCU2u zabbix
if [ $? -ne 0 ]; then
...
yum install httpd -y
yum install zabbix-web-mysql-$v -y
#configure timezone
...
yum install zabbix-agent-$v -y
yum install zabbix-sender-$v -y
yum install zabbix-get-$v -y
